...
namespace _20190624_基于WinForm的宿舍管理系统.DAL
{
    /// <summary>
    ...
    */
    abstract class BaseDAL<T> where T : new()
    {
        ...
        /// <returns></returns>
        public bool Drop(T bean, List<string> conditions)
        {
            //初始化
            DataTable dataTable = null;
        ...
        /// <returns></returns>
        public T FindModel(T bean, List<string> conditions)
        {
            //初始化
            DataTable dataTable = null;
            T t = default(T);        //在泛型类型中，引用类型的default将泛型类型初始化null，值类型的default将泛型类型初始化为0
        ...
        /// <param name="conditions"></param>
        /// <returns></returns>
        public List<T> FindList(T bean, List<string> conditions)
        {
            //初始化
            DataTable dataTable = null;
        ...
        /// <param name="conditions"></param>
        /// <returns></returns>
        protected string GenerateSql_Insert(T model)
        {
        ...
        /// <param name="conditions"></param>
        /// <returns></returns>
        protected string GenerateSql_Delete(T bean, List<string> conditions)
        {
            // 获取泛型的具体类型
            Type type = typeof(T);
            ...
            foreach (PropertyInfo pi in propertys) {
                ...
                foreach (string con in conditions) {
                    ...
                    {
                        //构造SQL字符串
                        string propValue = pi.GetValue(bean).ToString();//根据属性名，得到bean对象的属性值
        ...
        /// <param name="bean"></param>
        /// <returns></returns>
        protected string GenerateSql_Select(T bean, List<string> conditions)
        {
            // 获取泛型的具体类型
            Type type = typeof(T);
            ...
            //如果传入的conditions长度为零，则认为是一个无条件查询语句
            if (conditions.Count == 0) {
                return sql.ToString();
            }
